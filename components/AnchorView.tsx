...
import { Send, Loader, FileText, Paperclip, X, Phone, PhoneOff, Camera, Globe, Sparkles, Music } from './icons/Icons';
...
interface AnchorViewProps {
    ...
    onOpenMusicStudio?: (prompt: string) => void;
}

const AnchorView: React.FC<AnchorViewProps> = ({ session, onUpdateSession, onRenameSession, userName, onOpenImageStudio, onOpenMusicStudio }) => {
  ...
  const { connect, disconnect, isConnected, volume, error: liveError, transcription } = useGeminiLive(session.id, onOpenImageStudio, onOpenMusicStudio);
  ...
  const handleSend = async (e: React.FormEvent) => {
    ...
    // Pass isSearchMode and stageAssets
    const { text, sources, generatedAssets, openImageStudio, openMusicStudio } = await getChatResponse(messages, userText, filesToSend, userName, session.id, stageAssets, isSearchMode);
    await addMessage(session.id, 'model', text, undefined, sources);
    
    if (openImageStudio && onOpenImageStudio) {
        onOpenImageStudio(openImageStudio);
    }

    if (openMusicStudio && onOpenMusicStudio) {
        onOpenMusicStudio(openMusicStudio);
    }
      ...
      <div className="flex flex-col h-full w-full relative bg-transparent">
        ...
        <form onSubmit={handleSend} className="p-5 border-t border-white/10 bg-black/40 backdrop-blur-2xl flex-shrink-0 relative z-30 shadow-[0_-5px_30px_rgba(0,0,0,0.3)]">
          {isConnected && transcription && (
              <div className="absolute bottom-full left-0 right-0 p-3 bg-black/80 backdrop-blur-md border-t border-white/10 z-40 text-xs">
                  <span className={`font-bold ${transcription.role === 'user' ? 'text-blue-400' : 'text-talia-red'}`}>
                      {transcription.role === 'user' ? 'Você: ' : 'Talia: '}
                  </span>
                  <span className="text-gray-300">{transcription.text}</span>
              </div>
          )}
          ...
          <div className="flex gap-3 items-end">
            ...
            <button
                type="button"
                onClick={() => onOpenImageStudio && onOpenImageStudio("")}
                className="p-3 text-gray-400 hover:text-white transition-colors rounded-none hover:bg-white/5 border border-transparent hover:border-white/10"
                disabled={loading}
                title="Visual Studio"
            >
            ...
            <button
                type="button"
                onClick={() => onOpenMusicStudio && onOpenMusicStudio("")}
                className="p-3 text-gray-400 hover:text-white transition-colors rounded-none hover:bg-white/5 border border-transparent hover:border-white/10"
                disabled={loading}
                title="Music Studio"
            >
