...
import { Phone, PhoneOff, Sparkles, Clock, ChevronRight, Globe, Music } from './icons/Icons';
...
const TaliaCoreSidebar: React.FC<TaliaCorePanelProps> = ({ session, userName, autonomyMode, onRenameSession, onOpenImageStudio, onOpenMusicStudio }) => {
  ...
  const { connect, disconnect, isConnected, error: liveError, transcription, volume } = useGeminiLive(session.id, onOpenImageStudio, onOpenMusicStudio);
  ...
  const handleSend = async (e?: React.FormEvent) => {
    ...
    if (response.openMusicStudio) {
      onOpenMusicStudio(response.openMusicStudio);
    }
    ...
    <div className="w-full h-full bg-[#050507]/60 backdrop-blur-xl flex flex-col z-30 shadow-2xl relative border-l border-white/5 transition-all duration-300">
      ...
      <div className="flex-grow flex flex-col overflow-hidden relative">
         ...
         <div className="shrink-0 bg-black/40 backdrop-blur-xl border-t border-white/5 px-2 py-2 relative z-50">
            {liveError && (
                <div className="absolute bottom-full left-0 right-0 p-2 bg-red-900/90 text-white text-xs border-t border-red-500 z-50">
